...
// CreateFiles_Delete - создаёт 1 файл
func CreateFiles_Delete(MapAll map[string]*types.Table, Table1 *types.Table) error {
	var err error
	...
	FilenameTemplateDB := DirTemplatesDB + config.Settings.TEMPLATES_CRUD_DELETE_FILENAME
	...
	TextDB = CreateFiles_Delete1(TextDB, Table1)
...
// CreateFiles_Delete1 - заполняет Text
func CreateFiles_Delete1(Text string, Table1 *types.Table) string {
	Otvet := Text
