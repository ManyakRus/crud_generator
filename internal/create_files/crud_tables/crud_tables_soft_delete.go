...
// CreateFiles_Soft_Delete - создаёт 1 файл
func CreateFiles_Soft_Delete(MapAll map[string]*types.Table, Table1 *types.Table) error {
	var err error
	...
	TextDB = CreateFiles_Soft_Delete1(TextDB, Table1)
...
// CreateFiles_Soft_Delete1 - заполняет Text
func CreateFiles_Soft_Delete1(Text string, Table1 *types.Table) string {
	Otvet := Text
	...
	if create_files.Has_Column_DeletedAt_Time(Table1) == true {
		ReplaceDeletedAtEqual = ", deleted_at = now()"
		ReplaceFieldDeletedAt = "m.DeletedAt = time.Now()"
	}
