...
// CreateFiles_crud_manual - создаёт 1 файл в папке db
func CreateFiles_crud_manual(MapAll map[string]*types.Table, Table1 *types.Table) error {
	var err error
	...
	FilenameTemplateObject := DirTemplatesObjects + config.Settings.TEMPLATES_MODEL_CRUD_OBJECT_MANUAL_FILENAME
	TableName := strings.ToLower(Table1.Name)
	DirReadyTable := DirReadyDB + config.Settings.PREFIX_READOBJECT + TableName
	FilenameReadyObjects := DirReadyTable + micro.SeparatorFile() + config.Settings.PREFIX_READOBJECT + TableName + config.Settings.SUFFIX_CRUD + "_manual.go"
