...
func (crud Crud_GRPC) Read(m *banks.Bank) error {
	...
	// запрос
	Response, err := grpc_client.Client.Bank_Read(ctx, Request)
	if err != nil {
		sError := err.Error()
...
func (crud Crud_GRPC) Create(m *banks.Bank) error {
	// var Otvet banks.Bank
	...
	// запрос
	Response, err := grpc_client.Client.Bank_Create(ctx, Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_GRPC) Update(m *banks.Bank) error {
	// var Otvet banks.Bank
	...
	// запрос
	Response, err := grpc_client.Client.Bank_Update(ctx, Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_GRPC) Save(m *banks.Bank) error {
	// var Otvet banks.Bank
	...
	// запрос
	Response, err := grpc_client.Client.Bank_Save(ctx, Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_GRPC) Delete(m *banks.Bank) error {
	// var Otvet banks.Bank
	...
	// запрос
	Response, err := grpc_client.Client.Bank_Delete(ctx, Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_GRPC) Restore(m *banks.Bank) error {
	// var Otvet banks.Bank
	...
	// запрос
	Response, err := grpc_client.Client.Bank_Restore(ctx, Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_GRPC) Find_ByExtID(m *banks.Bank) error {
	//подключение
	if grpc_client.Client == nil {
		grpc_client.Connect()
	}
	...
	//запрос
	Response, err := grpc_client.Client.Bank_FindByExtID(ctx, Request)
	if err != nil {
		sError := err.Error()
