...
func (crud Crud_GRPC) GetVersionModel() uint32 {
	if VersionModel == 0 {
		VersionModel = connections.Connection{}.GetStructVersion()
	}
	return VersionModel
}

// Read - возвращает модель из БД
func (crud Crud_GRPC) Read(m *connections.Connection) error {
	// var Otvet connections.Connection
	...
	// запрос
	Response, err := grpc_client.Client.Connection_Read(ctx, Request)
	if err != nil {
		sError := err.Error()
...
func (crud Crud_GRPC) Create(m *connections.Connection) error {
	// var Otvet connections.Connection
	...
	// запрос
	Response, err := grpc_client.Client.Connection_Create(ctx, Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_GRPC) Update(m *connections.Connection) error {
	// var Otvet connections.Connection
	...
	// запрос
	Response, err := grpc_client.Client.Connection_Update(ctx, Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_GRPC) Save(m *connections.Connection) error {
	// var Otvet connections.Connection
	...
	// запрос
	Response, err := grpc_client.Client.Connection_Save(ctx, Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
