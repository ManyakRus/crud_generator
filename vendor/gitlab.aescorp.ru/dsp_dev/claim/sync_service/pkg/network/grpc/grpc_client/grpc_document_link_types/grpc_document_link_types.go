...
package grpc_document_link_types
...
func (crud Crud_GRPC) Read(m *document_link_types.DocumentLinkType) error {
	...
	// запрос
	Response, err := grpc_client.Client.DocumentLinkType_Read(ctx, Request)
	if err != nil {
		sError := err.Error()
...
func (crud Crud_GRPC) Create(m *document_link_types.DocumentLinkType) error {
	// var Otvet document_link_types.DocumentLinkType
	...
	// запрос
	Response, err := grpc_client.Client.DocumentLinkType_Create(ctx, Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_GRPC) Update(m *document_link_types.DocumentLinkType) error {
	// var Otvet document_link_types.DocumentLinkType
	...
	// запрос
	Response, err := grpc_client.Client.DocumentLinkType_Update(ctx, Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_GRPC) Save(m *document_link_types.DocumentLinkType) error {
	// var Otvet document_link_types.DocumentLinkType
	...
	// запрос
	Response, err := grpc_client.Client.DocumentLinkType_Save(ctx, Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_GRPC) Delete(m *document_link_types.DocumentLinkType) error {
	// var Otvet document_link_types.DocumentLinkType
	...
	// запрос
	Response, err := grpc_client.Client.DocumentLinkType_Delete(ctx, Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_GRPC) Restore(m *document_link_types.DocumentLinkType) error {
	// var Otvet document_link_types.DocumentLinkType
	...
	// запрос
	Response, err := grpc_client.Client.DocumentLinkType_Restore(ctx, Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
