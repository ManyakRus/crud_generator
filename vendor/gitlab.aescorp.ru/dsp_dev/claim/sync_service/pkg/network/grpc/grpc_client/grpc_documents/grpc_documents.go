...
func (crud Crud_GRPC) Read(m *documents.Document) error {
	...
	// запрос
	Response, err := grpc_client.Client.Document_Read(ctx, Request)
	if err != nil {
		sError := err.Error()
...
func (crud Crud_GRPC) Create(m *documents.Document) error {
	// var Otvet documents.Document
	...
	// запрос
	Response, err := grpc_client.Client.Document_Create(ctx, Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_GRPC) Update(m *documents.Document) error {
	// var Otvet documents.Document
	...
	// запрос
	Response, err := grpc_client.Client.Document_Update(ctx, Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_GRPC) Save(m *documents.Document) error {
	// var Otvet documents.Document
	...
	// запрос
	Response, err := grpc_client.Client.Document_Save(ctx, Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_GRPC) Delete(m *documents.Document) error {
	// var Otvet documents.Document
	...
	// запрос
	Response, err := grpc_client.Client.Document_Delete(ctx, Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_GRPC) Restore(m *documents.Document) error {
	// var Otvet documents.Document
	...
	// запрос
	Response, err := grpc_client.Client.Document_Restore(ctx, Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_GRPC) Find_ByExtID(m *documents.Document) error {
	//подключение
	if grpc_client.Client == nil {
		grpc_client.Connect()
	}
	...
	//запрос
	Response, err := grpc_client.Client.Document_FindByExtID(ctx, Request)
	if err != nil {
		sError := err.Error()
