...
func (crud Crud_GRPC) GetVersionModel() uint32 {
	if VersionModel == 0 {
		VersionModel = event_types.EventType{}.GetStructVersion()
	}
	return VersionModel
}

// Read - возвращает модель из БД
func (crud Crud_GRPC) Read(m *event_types.EventType) error {
	// var Otvet event_types.EventType
	...
	// запрос
	Response, err := grpc_client.Client.EventType_Read(ctx, Request)
	if err != nil {
		sError := err.Error()
...
func (crud Crud_GRPC) Create(m *event_types.EventType) error {
	// var Otvet event_types.EventType
	...
	// запрос
	Response, err := grpc_client.Client.EventType_Create(ctx, Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_GRPC) Update(m *event_types.EventType) error {
	// var Otvet event_types.EventType
	...
	// запрос
	Response, err := grpc_client.Client.EventType_Update(ctx, Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_GRPC) Save(m *event_types.EventType) error {
	// var Otvet event_types.EventType
	...
	// запрос
	Response, err := grpc_client.Client.EventType_Save(ctx, Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_GRPC) Delete(m *event_types.EventType) error {
	// var Otvet event_types.EventType
	...
	// запрос
	Response, err := grpc_client.Client.EventType_Delete(ctx, Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_GRPC) Restore(m *event_types.EventType) error {
	// var Otvet event_types.EventType
	...
	// запрос
	Response, err := grpc_client.Client.EventType_Restore(ctx, Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
