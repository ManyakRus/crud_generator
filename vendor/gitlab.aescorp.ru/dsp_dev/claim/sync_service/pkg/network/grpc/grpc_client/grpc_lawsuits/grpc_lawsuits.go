...
func (crud Crud_GRPC) GetVersionModel() uint32 {
	if VersionModel == 0 {
		VersionModel = lawsuits.Lawsuit{}.GetStructVersion()
	}
	return VersionModel
}

// Read - возвращает модель из БД
func (crud Crud_GRPC) Read(m *lawsuits.Lawsuit) error {
	// var Otvet lawsuits.Lawsuit
	...
	// запрос
	Response, err := grpc_client.Client.Lawsuit_Read(ctx, Request)
	if err != nil {
		sError := err.Error()
...
func (crud Crud_GRPC) Create(m *lawsuits.Lawsuit) error {
	// var Otvet lawsuits.Lawsuit
	...
	// запрос
	Response, err := grpc_client.Client.Lawsuit_Create(ctx, Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_GRPC) Update(m *lawsuits.Lawsuit) error {
	// var Otvet lawsuits.Lawsuit
	...
	// запрос
	Response, err := grpc_client.Client.Lawsuit_Update(ctx, Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_GRPC) Save(m *lawsuits.Lawsuit) error {
	// var Otvet lawsuits.Lawsuit
	...
	// запрос
	Response, err := grpc_client.Client.Lawsuit_Save(ctx, Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_GRPC) Delete(m *lawsuits.Lawsuit) error {
	// var Otvet lawsuits.Lawsuit
	...
	// запрос
	Response, err := grpc_client.Client.Lawsuit_Delete(ctx, Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_GRPC) Restore(m *lawsuits.Lawsuit) error {
	// var Otvet lawsuits.Lawsuit
	...
	// запрос
	Response, err := grpc_client.Client.Lawsuit_Restore(ctx, Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_GRPC) Find_ByExtID(m *lawsuits.Lawsuit) error {
	//подключение
	if grpc_client.Client == nil {
		grpc_client.Connect()
	}
	...
	//запрос
	Response, err := grpc_client.Client.Lawsuit_FindByExtID(ctx, Request)
	if err != nil {
		sError := err.Error()
