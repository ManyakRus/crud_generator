...
package grpc_message_send_statuses
...
func (crud Crud_GRPC) Read(m *message_send_statuses.MessageSendStatus) error {
	...
	// запрос
	Response, err := grpc_client.Client.MessageSendStatus_Read(ctx, Request)
	if err != nil {
		sError := err.Error()
...
func (crud Crud_GRPC) Create(m *message_send_statuses.MessageSendStatus) error {
	// var Otvet message_send_statuses.MessageSendStatus
	...
	// запрос
	Response, err := grpc_client.Client.MessageSendStatus_Create(ctx, Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_GRPC) Update(m *message_send_statuses.MessageSendStatus) error {
	// var Otvet message_send_statuses.MessageSendStatus
	...
	// запрос
	Response, err := grpc_client.Client.MessageSendStatus_Update(ctx, Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_GRPC) Save(m *message_send_statuses.MessageSendStatus) error {
	// var Otvet message_send_statuses.MessageSendStatus
	...
	// запрос
	Response, err := grpc_client.Client.MessageSendStatus_Save(ctx, Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_GRPC) Delete(m *message_send_statuses.MessageSendStatus) error {
	// var Otvet message_send_statuses.MessageSendStatus
	...
	// запрос
	Response, err := grpc_client.Client.MessageSendStatus_Delete(ctx, Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_GRPC) Restore(m *message_send_statuses.MessageSendStatus) error {
	// var Otvet message_send_statuses.MessageSendStatus
	...
	// запрос
	Response, err := grpc_client.Client.MessageSendStatus_Restore(ctx, Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
