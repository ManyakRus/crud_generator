...
package grpc_payment_schedules
...
func (crud Crud_GRPC) GetVersionModel() uint32 {
	if VersionModel == 0 {
		VersionModel = payment_schedules.PaymentSchedule{}.GetStructVersion()
	}
	return VersionModel
}

// Read - возвращает модель из БД
func (crud Crud_GRPC) Read(m *payment_schedules.PaymentSchedule) error {
	// var Otvet payment_schedules.PaymentSchedule
	...
	// запрос
	Response, err := grpc_client.Client.PaymentSchedule_Read(ctx, Request)
	if err != nil {
		sError := err.Error()
...
func (crud Crud_GRPC) Create(m *payment_schedules.PaymentSchedule) error {
	// var Otvet payment_schedules.PaymentSchedule
	...
	// запрос
	Response, err := grpc_client.Client.PaymentSchedule_Create(ctx, Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_GRPC) Update(m *payment_schedules.PaymentSchedule) error {
	// var Otvet payment_schedules.PaymentSchedule
	...
	// запрос
	Response, err := grpc_client.Client.PaymentSchedule_Update(ctx, Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_GRPC) Save(m *payment_schedules.PaymentSchedule) error {
	// var Otvet payment_schedules.PaymentSchedule
	...
	// запрос
	Response, err := grpc_client.Client.PaymentSchedule_Save(ctx, Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_GRPC) Delete(m *payment_schedules.PaymentSchedule) error {
	// var Otvet payment_schedules.PaymentSchedule
	...
	// запрос
	Response, err := grpc_client.Client.PaymentSchedule_Delete(ctx, Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_GRPC) Restore(m *payment_schedules.PaymentSchedule) error {
	// var Otvet payment_schedules.PaymentSchedule
	...
	// запрос
	Response, err := grpc_client.Client.PaymentSchedule_Restore(ctx, Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_GRPC) Find_ByExtID(m *payment_schedules.PaymentSchedule) error {
	//подключение
	if grpc_client.Client == nil {
		grpc_client.Connect()
	}
	...
	//запрос
	Response, err := grpc_client.Client.PaymentSchedule_FindByExtID(ctx, Request)
	if err != nil {
		sError := err.Error()
