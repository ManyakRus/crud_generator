...
package grpc_penalty_calculation_items
...
func (crud Crud_GRPC) GetVersionModel() uint32 {
	if VersionModel == 0 {
		VersionModel = penalty_calculation_items.PenaltyCalculationItem{}.GetStructVersion()
	}
	return VersionModel
}

// Read - возвращает модель из БД
func (crud Crud_GRPC) Read(m *penalty_calculation_items.PenaltyCalculationItem) error {
	// var Otvet penalty_calculation_items.PenaltyCalculationItem
	...
	// запрос
	Response, err := grpc_client.Client.PenaltyCalculationItem_Read(ctx, Request)
	if err != nil {
		sError := err.Error()
...
func (crud Crud_GRPC) Create(m *penalty_calculation_items.PenaltyCalculationItem) error {
	// var Otvet penalty_calculation_items.PenaltyCalculationItem
	...
	// запрос
	Response, err := grpc_client.Client.PenaltyCalculationItem_Create(ctx, Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_GRPC) Update(m *penalty_calculation_items.PenaltyCalculationItem) error {
	// var Otvet penalty_calculation_items.PenaltyCalculationItem
	...
	// запрос
	Response, err := grpc_client.Client.PenaltyCalculationItem_Update(ctx, Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_GRPC) Save(m *penalty_calculation_items.PenaltyCalculationItem) error {
	// var Otvet penalty_calculation_items.PenaltyCalculationItem
	...
	// запрос
	Response, err := grpc_client.Client.PenaltyCalculationItem_Save(ctx, Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_GRPC) Delete(m *penalty_calculation_items.PenaltyCalculationItem) error {
	// var Otvet penalty_calculation_items.PenaltyCalculationItem
	...
	// запрос
	Response, err := grpc_client.Client.PenaltyCalculationItem_Delete(ctx, Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_GRPC) Restore(m *penalty_calculation_items.PenaltyCalculationItem) error {
	// var Otvet penalty_calculation_items.PenaltyCalculationItem
	...
	// запрос
	Response, err := grpc_client.Client.PenaltyCalculationItem_Restore(ctx, Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_GRPC) Find_ByExtID(m *penalty_calculation_items.PenaltyCalculationItem) error {
	//подключение
	if grpc_client.Client == nil {
		grpc_client.Connect()
	}
	...
	//запрос
	Response, err := grpc_client.Client.PenaltyCalculationItem_FindByExtID(ctx, Request)
	if err != nil {
		sError := err.Error()
