...
package grpc_service_providers
...
func (crud Crud_GRPC) GetVersionModel() uint32 {
	if VersionModel == 0 {
		VersionModel = service_providers.ServiceProvider{}.GetStructVersion()
	}
	return VersionModel
}

// Read - возвращает модель из БД
func (crud Crud_GRPC) Read(m *service_providers.ServiceProvider) error {
	// var Otvet service_providers.ServiceProvider
	...
	// запрос
	Response, err := grpc_client.Client.ServiceProvider_Read(ctx, Request)
	if err != nil {
		sError := err.Error()
...
func (crud Crud_GRPC) Create(m *service_providers.ServiceProvider) error {
	// var Otvet service_providers.ServiceProvider
	...
	// запрос
	Response, err := grpc_client.Client.ServiceProvider_Create(ctx, Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_GRPC) Update(m *service_providers.ServiceProvider) error {
	// var Otvet service_providers.ServiceProvider
	...
	// запрос
	Response, err := grpc_client.Client.ServiceProvider_Update(ctx, Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_GRPC) Save(m *service_providers.ServiceProvider) error {
	// var Otvet service_providers.ServiceProvider
	...
	// запрос
	Response, err := grpc_client.Client.ServiceProvider_Save(ctx, Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_GRPC) Delete(m *service_providers.ServiceProvider) error {
	// var Otvet service_providers.ServiceProvider
	...
	// запрос
	Response, err := grpc_client.Client.ServiceProvider_Delete(ctx, Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_GRPC) Restore(m *service_providers.ServiceProvider) error {
	// var Otvet service_providers.ServiceProvider
	...
	// запрос
	Response, err := grpc_client.Client.ServiceProvider_Restore(ctx, Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_GRPC) Find_ByExtID(m *service_providers.ServiceProvider) error {
	//подключение
	if grpc_client.Client == nil {
		grpc_client.Connect()
	}
	...
	//запрос
	Response, err := grpc_client.Client.ServiceProvider_FindByExtID(ctx, Request)
	if err != nil {
		sError := err.Error()
