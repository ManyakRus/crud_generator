...
func (crud Crud_NRPC) GetVersionModel() uint32 {
	if VersionModel == 0 {
		VersionModel = accounting_areas.AccountingArea{}.GetStructVersion()
	}
	return VersionModel
}

// Read - возвращает модель из БД
func (crud Crud_NRPC) Read(m *accounting_areas.AccountingArea) error {
	// var Otvet accounting_areas.AccountingArea
	...
	// запрос
	Response, err := nrpc_client.Client.AccountingArea_Read(Request)
	if err != nil {
		sError := err.Error()
...
func (crud Crud_NRPC) Create(m *accounting_areas.AccountingArea) error {
	// var Otvet accounting_areas.AccountingArea
	...
	// запрос
	Response, err := nrpc_client.Client.AccountingArea_Create(Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_NRPC) Update(m *accounting_areas.AccountingArea) error {
	// var Otvet accounting_areas.AccountingArea
	...
	// запрос
	Response, err := nrpc_client.Client.AccountingArea_Update(Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_NRPC) Save(m *accounting_areas.AccountingArea) error {
	// var Otvet accounting_areas.AccountingArea
	...
	// запрос
	Response, err := nrpc_client.Client.AccountingArea_Save(Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_NRPC) Delete(m *accounting_areas.AccountingArea) error {
	// var Otvet accounting_areas.AccountingArea
	...
	// запрос
	Response, err := nrpc_client.Client.AccountingArea_Delete(Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_NRPC) Restore(m *accounting_areas.AccountingArea) error {
	// var Otvet accounting_areas.AccountingArea
	...
	// запрос
	Response, err := nrpc_client.Client.AccountingArea_Restore(Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_NRPC) Find_ByExtID(m *accounting_areas.AccountingArea) error {
	//подключение
	if nrpc_client.Client == nil {
		nrpc_client.Connect()
	}
	...
	//запрос
	Response, err := nrpc_client.Client.AccountingArea_FindByExtID(Request)
	if err != nil {
		sError := err.Error()
