...
func (crud Crud_NRPC) GetVersionModel() uint32 {
	if VersionModel == 0 {
		VersionModel = document_links.DocumentLink{}.GetStructVersion()
	}
	return VersionModel
}

// Read - возвращает модель из БД
func (crud Crud_NRPC) Read(m *document_links.DocumentLink) error {
	// var Otvet document_links.DocumentLink
	...
	// запрос
	Response, err := nrpc_client.Client.DocumentLink_Read(Request)
	if err != nil {
		sError := err.Error()
...
func (crud Crud_NRPC) Create(m *document_links.DocumentLink) error {
	// var Otvet document_links.DocumentLink
	...
	// запрос
	Response, err := nrpc_client.Client.DocumentLink_Create(Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_NRPC) Update(m *document_links.DocumentLink) error {
	// var Otvet document_links.DocumentLink
	...
	// запрос
	Response, err := nrpc_client.Client.DocumentLink_Update(Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_NRPC) Save(m *document_links.DocumentLink) error {
	// var Otvet document_links.DocumentLink
	...
	// запрос
	Response, err := nrpc_client.Client.DocumentLink_Save(Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_NRPC) Delete(m *document_links.DocumentLink) error {
	// var Otvet document_links.DocumentLink
	...
	// запрос
	Response, err := nrpc_client.Client.DocumentLink_Delete(Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_NRPC) Restore(m *document_links.DocumentLink) error {
	// var Otvet document_links.DocumentLink
	...
	// запрос
	Response, err := nrpc_client.Client.DocumentLink_Restore(Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_NRPC) Find_ByExtID(m *document_links.DocumentLink) error {
	//подключение
	if nrpc_client.Client == nil {
		nrpc_client.Connect()
	}
	...
	//запрос
	Response, err := nrpc_client.Client.DocumentLink_FindByExtID(Request)
	if err != nil {
		sError := err.Error()
