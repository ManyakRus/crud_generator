...
func (crud Crud_NRPC) Read(m *file_changes.FileChange) error {
	...
	// запрос
	Response, err := nrpc_client.Client.FileChange_Read(Request)
	if err != nil {
		sError := err.Error()
...
func (crud Crud_NRPC) Create(m *file_changes.FileChange) error {
	// var Otvet file_changes.FileChange
	...
	// запрос
	Response, err := nrpc_client.Client.FileChange_Create(Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_NRPC) Update(m *file_changes.FileChange) error {
	// var Otvet file_changes.FileChange
	...
	// запрос
	Response, err := nrpc_client.Client.FileChange_Update(Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_NRPC) Save(m *file_changes.FileChange) error {
	// var Otvet file_changes.FileChange
	...
	// запрос
	Response, err := nrpc_client.Client.FileChange_Save(Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
