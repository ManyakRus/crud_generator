...
func (crud Crud_NRPC) Read(m *individuals.Individual) error {
	...
	// запрос
	Response, err := nrpc_client.Client.Individual_Read(Request)
	if err != nil {
		sError := err.Error()
...
func (crud Crud_NRPC) Create(m *individuals.Individual) error {
	// var Otvet individuals.Individual
	...
	// запрос
	Response, err := nrpc_client.Client.Individual_Create(Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_NRPC) Update(m *individuals.Individual) error {
	// var Otvet individuals.Individual
	...
	// запрос
	Response, err := nrpc_client.Client.Individual_Update(Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_NRPC) Save(m *individuals.Individual) error {
	// var Otvet individuals.Individual
	...
	// запрос
	Response, err := nrpc_client.Client.Individual_Save(Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_NRPC) Delete(m *individuals.Individual) error {
	// var Otvet individuals.Individual
	...
	// запрос
	Response, err := nrpc_client.Client.Individual_Delete(Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_NRPC) Restore(m *individuals.Individual) error {
	// var Otvet individuals.Individual
	...
	// запрос
	Response, err := nrpc_client.Client.Individual_Restore(Request)
	if err != nil {
		sError := err.Error()
		if len(sError) >= len(constants.TEXT_ERROR_MODEL_VERSION) && sError[0:len(constants.TEXT_ERROR_MODEL_VERSION)] == constants.TEXT_ERROR_MODEL_VERSION {
			log.Panic("table: ", TableName, " error: ", err)
...
func (crud Crud_NRPC) Find_ByExtID(m *individuals.Individual) error {
	//подключение
	if nrpc_client.Client == nil {
		nrpc_client.Connect()
	}
	...
	//запрос
	Response, err := nrpc_client.Client.Individual_FindByExtID(Request)
	if err != nil {
		sError := err.Error()
